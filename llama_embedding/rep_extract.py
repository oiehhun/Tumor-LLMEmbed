...
import torch
import json
from transformers import AutoConfig, AutoModelForCausalLM, AutoTokenizer, LlamaForSequenceClassification, BitsAndBytesConfig
from tqdm import trange
from datasets import load_dataset
import argparse
import pandas as pd

def rep_extract(task, mode, device, sents, labels, max_len, step, args=None):
    ...
    for idx in trange(0, len(sents), step):
        ...
        with torch.no_grad():
            ...
            reps_batch_5L = []
            for layer in range(-1, -(args.num_layer + 1), -1):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--task', type=str, default='tumor')   # sst2, mr, agnews, r8, r52, tumor
    parser.add_argument('--num_layer', type=int,default=5, help='추출할 레이어 수')
    ...
    if task == 'sst2':
        ...
        rep_extract(task, 'train', device, sents, labels, 1024, 2, args)
        
        dataset = pd.read_csv('../data/tumor/tumor_test.csv')
        sents = dataset['Reports'].tolist()
        labels = dataset['label'].tolist()
        rep_extract(task, 'test', device, sents, labels, 1024, 2, args)